...
export async function getUserEntitlements(userId) {
  if (!userId) {
    return { plan: 'free', ...PLAN_LIMITS.free, lifetimeSlots: 0, totalPublishSlots: 0 }
  }

  // 1. Look up an active subscription, if any.
  ...
  // 2. Sum up lifetime portfolio slots from paid one-time purchases.
